...
/**
 * Swing组件从功能上分可分为： 1) 顶层容器：JFrame,JApplet,JDialog,JWindow共4个 2)
 * 中间容器：如：JPanel,JScrollPane,JSplitPane,JToolBar 3)
 * 特殊容器：在GUI上起特殊作用的中间层，如JInternalFrame,JLayeredPane,JRootPane. 4)
 * 基本控件：实现人际交互的组件，如JButton, JComboBox, JList, JMenu, JSlider, JtextField。 5)
 * 不可编辑信息的显示：向用户显示不可编辑信息的组件，例如JLabel, JProgressBar, ToolTip。 6)
 * 可编辑信息的显示：向用户显示能被编辑的格式化信息的组件，如JColorChooser, JFileChoose, JFileChooser,
 * Jtable, JtextArea
 */
