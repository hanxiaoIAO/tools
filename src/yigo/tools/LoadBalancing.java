...
public class LoadBalancing {
	...
	static final String propeitiesTextMould = "DSNTag=\n" + "Name=" + NAMESTRING + "\n" + "ConnectionType=jdbc\n"
			...
			+ "DriverExt=com.mysql.jdbc.Driver\n" + "URL=jdbc:mysql://" + IP + ":3306/" + NAMESTRING
			+ "?useUnicode=true&amp;characterEncoding=UTF-8\n" + "User=root\n" + "Password=111111\n" + "ExtConfig=xa\n";
	static final String DatabaseName = "DatabaseName";
	static final String createDataBaseStringMoudle = "create DataBase " + DatabaseName + "\n";
	...
	private static void puechaseMethod() throws Exception {
		...
		for (String purchase : peuchases) {
			for (int date : dates) {
				...
				propeitiesText = propeitiesText.replaceAll(NAMESTRING, fileName);
				propeitiesText = propeitiesText.replace(IP, ip);
				WriteFileTools.WriteToFile(propeitiesText, path + fileName + ".properties");

				DSNNamed = DSNNamed + fileName + ";";
				createDataBaseString = createDataBaseString
						+ createDataBaseStringMoudle.replaceAll(DatabaseName, fileName);
			}
		}
		createDataBaseString = createDataBaseString + "\n\n\n";
		DSNNamed = DSNNamed + "\n\n\n";
	...
	public static void main(String[] args) throws Exception {
		puechaseMethod();
		companyMethod();
		WriteFileTools.WriteToFile(createDataBaseString, path + createDataBaseSQL);
		WriteFileTools.WriteToFile(DSNNamed, path + DSNproperties);
	}
